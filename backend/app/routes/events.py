...
def validate_details(
    *,
    ...
    if event_type.supports_location:
        if location == "NOT_APPLICABLE":
            raise HTTPException(status_code=400, detail="Choose inside or outside.")
    elif location != "NOT_APPLICABLE":
        raise HTTPException(status_code=400, detail="This event does not support a location.")

    option = None
    if event_type.option_category:
        if event_type.option_required and option_id is None:
            label = event_type.option_category.lower().replace("_", " ")
            raise HTTPException(status_code=400, detail=f"Choose a {label}.")
    ...
    if event_type.numeric_required and numeric_value is None:
        raise HTTPException(
            status_code=400,
            detail=f"{event_type.numeric_label or 'Value'} is required.",
        )
    ...
    if numeric_value is not None and event_type.allowed_units and unit not in event_type.allowed_units:
...
def get_event_context(
    ...
    require_owned_dog(db, event.dog_id, user)
...
def list_events(
    ...
    return [
        build_event_response(event, event_type, option)
        for event, event_type, option in db.execute(statement).all()
    ]
...
def update_event(
    ...
    event, event_type = get_event_context(db, event_id, user)
    values = data.model_dump(exclude_unset=True)
    effective = {
        "state": values.get("state", event.state),
        "location": values.get("location", event.location),
...
def delete_event(
    ...
    event, _ = get_event_context(db, event_id, user)
    db.delete(event)
    db.commit()
...
def validate_event_time(event_time: datetime) -> None:
    ...
    if event_time > now + timedelta(minutes=5):
        raise HTTPException(status_code=400, detail="Event time cannot be in the future.")
